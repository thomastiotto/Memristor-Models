...
with Timer(title="solve_ivp"):
    ...
    solutions.append((x_solve_ivp_fitted.y[0, :], x_solve_ivp_fitted.t, "solve_ivp"))

# Plot reconstructed data
fig3, axes = plt.subplots(1, 2, figsize=(10, 4))
ax11 = axes[0]
ax11.plot(x_solve_ivp_fitted.t, x_solve_ivp_fitted.y[0, :], color="b")
ax11.set_ylabel(r'Current ($\mu A$)', color='b')
# ax11.set_ylim([-30, 30])
ax11.tick_params('y', colors='b')
ax12 = ax11.twinx()
ax12.plot(x_solve_ivp_fitted.t, V(x_solve_ivp_fitted.t), color="r")
ax12.set_ylabel('Voltage (V)', color='r')
ax12.tick_params('y', colors='r')
# ax12.set_ylim([-1.5, 1.5])
ax2 = axes[1]
ax2.plot(V(x_solve_ivp_fitted.t), x_solve_ivp_fitted.y[0, :])
# ax2.set_ylim([-25, 25])
ax2.set_ylabel(r'Current ($\mu A$)')
ax2.set_xlabel('Voltage (V)')
