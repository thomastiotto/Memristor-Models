...
def find_sensitivity(sign, params, idx, case, model, name):
    ...
    time, current_v, current_i, current_r, current_x = \
        model_sim_with_params(pulse_length=0.001,
                              resetV=resetV, numreset=10,
                              setV=setV, numset=10,
                              readV=readV, read_length=read_length,
                              init_set_length=120, init_setV=1,
                              **model)
    peaks_gt = find_peaks(current_r, current_v, readV, consider_from=120, debug=False)
    ...
    while count_iter < 500:  # Run until the error is about 10% or 500 iterations pass.
        ...
        if case == "I":
            ...
            time, voltage, i, r, x = model_sim_with_params(pulse_length=0.001,
                                                           ...
                                                           init_set_length=120, init_setV=1,
                                                           **new_model)
            peaks_new = find_peaks(r, voltage, readV=readV, consider_from=120, debug=False)
        else:
        ...
        change = np.sum(np.abs(peaks_gt - peaks_new) / np.abs(peaks_gt)) / (len(peaks_gt) + 1) * 100
        # print("change:", change)
