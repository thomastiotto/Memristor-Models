...
for V_neg in tqdm(np.linspace(-4, -4, 1), position=0, leave=False, desc='RESET Sweep', colour='green', ncols=80):
    R_pos = [R0]
    R_neg = [R0]
    X_pos = 0
    X_neg = 0

    for i in tqdm(range(5000), position=1, leave=False, desc='Pulse simulation', colour='red', ncols=80):
        v_pos = generate_wave({"t_rise": .001, "t_on": .1, "t_fall": .001, "t_off": .4, "V_on": V_pos, "V_off": -.1,
                               "n_cycles": 1}, model['dt'], 0)[1]
        v_neg = generate_wave(
            {"t_rise": .001, "t_on": .1, "t_fall": .001, "t_off": .4, "V_on": V_neg, "V_off": -.1,
             "n_cycles": 1}, model['dt'], 0)[1]
